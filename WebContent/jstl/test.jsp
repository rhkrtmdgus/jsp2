...
<ul>
	...
	<li>el 은 expression language. 기호는 $&#123;&#125; 사용합니다.</li>
	<li>jstl은 el과 함께 사용됩니다.</li>
	<li> <%-- <%= %> --%> 기호 대신 사용합니다. 애트리뷰트 getAttribute 메소드 동작 실행 후 출력 </li>
</ul>
...
<c:choose>
	...
	<c:when test="${age < 20 }"> <!-- 첫번째 조건이 거짓일 때만 실행합니다. (else if) -->
...
<c:forEach var="x" begin="10" end="30" step="2" varStatus="status"><!-- varStatus는 현재 상태 저장할 변수 -->
	<c:out value="${x }"/>, index = <c:out value="${status.count }"/> <br> <!-- varStatus의 속성은 1부터 시작하여 몇번째 값인지 -->
